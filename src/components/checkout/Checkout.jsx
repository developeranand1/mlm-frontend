...
export default function Checkout() {
  ...
  const { ready, isLoggedIn, token } = useAuth(); // ✅ ADD
  ...
  // 🔐 LOGIN CHECK (YAHI MAIN LOGIC HAI)
  useEffect(() => {
    if (!ready) return;
  ...
  // ⛔ jab tak auth ready na ho tab tak kuch render mat karo
  ...
  const payNow = async () => {
    ...
    if (!token) {
      setMsg({ type: "danger", text: "Please login again." });
      return;
    }
      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            ...
            setMsg({
              type: "success",
              text: "Payment verified & order placed ✅",
            });
            clearCart();
          } catch (err) {
